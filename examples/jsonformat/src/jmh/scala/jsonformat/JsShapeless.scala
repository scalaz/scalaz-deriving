...
object DerivedJsEncoder extends DerivedJsEncoder1 {
  ...
  implicit def cconsAnnotated[
    ...
    new DerivedJsEncoder[A, FieldType[K, H] :+: T, None.type :: J] {
      private[this] val hint =
        A().field.getOrElse("type") -> JsString(K.value.name)
...
private[jsonformat] trait DerivedJsEncoder1 extends DerivedJsEncoder2 {
  ...
  implicit def hcons[A, K <: Symbol, H, T <: HList, J <: HList](implicit
    K: Witness.Aux[K],
    H: Lazy[JsEncoder[H]],
    T: DerivedJsEncoder[A, T, J]
  ): DerivedJsEncoder[A, FieldType[K, H] :: T, None.type :: J] =
    new DerivedJsEncoder[A, FieldType[K, H] :: T, None.type :: J] {
      private[this] val field = K.value.name
      def toJsFields(ht: FieldType[K, H] :: T, anns: None.type :: J) =
        ht match {
          case head :: tail =>
  ...
  implicit def hconsCustom[A, K <: Symbol, H, T <: HList, J <: HList](implicit
    K: Witness.Aux[K],
    H: Lazy[JsEncoder[H]],
    T: DerivedJsEncoder[A, T, J]
  ): DerivedJsEncoder[A, FieldType[K, H] :: T, Some[json] :: J] =
  ...
  implicit def ccons[A, K <: Symbol, H, T <: Coproduct, J <: HList](implicit
    K: Witness.Aux[K],
    H: Lazy[JsEncoder[H]],
    T: DerivedJsEncoder[A, T, J]
  ): DerivedJsEncoder[A, FieldType[K, H] :+: T, None.type :: J] =
    new DerivedJsEncoder[A, FieldType[K, H] :+: T, None.type :: J] {
      private[this] val hint = ("type" -> JsString(K.value.name))
      def toJsFields(ht: FieldType[K, H] :+: T, anns: None.type :: J) =
        ht match {
          case Inl(head) =>
            H.value.toJson(head) match {
...
object DerivedCoproductJsDecoder extends DerivedCoproductJsDecoder1 {
  ...
  implicit def cconsAnnotated[
    ...
    new DerivedCoproductJsDecoder[A, FieldType[K, H] :+: T, None.type :: J] {
      private[this] val hintfield = A().field.getOrElse("type")
      private[this] val hint = K.value.name
...
object DerivedProductJsDecoder extends DerivedProductJsDecoder1 {
  ...
  def gen[A, R, N <: Nat, J <: HList, D <: HList](implicit
    G: LabelledGeneric.Aux[A, R],
    J: Annotations.Aux[json, A, J],
  ...
  implicit def hcons[
  ...
  ): DerivedProductJsDecoder[A, FieldType[K, H] :: T, None.type :: J, Option[
    H
  ] :: D] =
    new DerivedProductJsDecoder[
      ...
      private[this] val fieldname = K.value.name
      def fromJsObject(
        j: FastJsObject,
        anns: None.type :: J,
        defaults: Option[H] :: D
  ...
  implicit def hconsCustom[
    ...
    new DerivedProductJsDecoder[
      ...
      private[this] val err = s"missing field '$field'".left[H]
...
private[jsonformat] trait DerivedProductJsDecoder1 {
  ...
  implicit def hconsTagged[
    ...
    T: DerivedProductJsDecoder[A, T, J, D]
  ): DerivedProductJsDecoder[A, FieldType[
    K,
    H @@ Z
  ] :: T, None.type :: J, Option[
    H @@ Z
  ] :: D] = hcons(K, H, T)
