...
object MagnoliaEqual {
  ...
  def combine[T](ctx: CaseClass[Equal, T]): Equal[T] = new Equal[T] {
    def equal(a1: T, a2: T) = ctx.parameters.forall { p =>
  ...
  def dispatch[T](ctx: SealedTrait[Equal, T]): Equal[T] = new Equal[T] {
    def equal(a1: T, a2: T): Boolean = quick(a1, a2) || ctx.dispatch(a1) {
      sub =>
        sub.cast
