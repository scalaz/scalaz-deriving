...
object CopH {
  ...
  def unsafeApply[L <: TListH, H[_[_]], F[_]](
    index: Int,
    hf: H[F]
  ): CopH[L, F] =
    new CopH[L, F](index, hf)

  sealed abstract class Inject[H[_[_]], J[F[_]] <: CopH[?, F]] {
  ...
  object Inject {
    def apply[H[_[_]], J[F[_]] <: CopH[?, F]](implicit
      ev: Inject[H, J]
    ): Inject[H, J] = ev

    implicit def injectFromInjectL[H[_[_]], L <: TListH](implicit
      ev: InjectL[H, L]
    ): Inject[H, ({ type y[x[_]] = CopH[L, x] })#y] =
      new Inject[H, ({ type y[x[_]] = CopH[L, x] })#y] {
  ...
  final class RemoveL[H[_[_]], L <: TListH] private[RemoveL] (index: Int) {
    def apply[F[_]](
      c: CopH[L, F]
    ): Either[CopH[TListH.Op.Remove[H, L], F], H[F]] =
      Either.cond(
        c.index == index,
