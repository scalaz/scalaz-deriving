...
object DerivedOrder {
  ...
  implicit def hcons[H, T <: HList](
    implicit H: Lazy[Order[H]],
    T: DerivedOrder[T]
  ): DerivedOrder[H :: T] = new DerivedOrder[H :: T] {
  ...
  implicit def ccons[H, T <: Coproduct](
    ...
    override def equal(ht1: H :+: T, ht2: H :+: T): Boolean = (ht1, ht2) match {
      case (Inl(c1), Inl(c2)) => quick(c1, c2) || H.value.equal(c1, c2)
      case (Inr(c1), Inr(c2)) => T.equal(c1, c2)
      case _                  => false
    }
    def order(ht1: H :+: T, ht2: H :+: T): Ordering = (ht1, ht2) match {
      case (Inl(c1), Inl(c2)) =>
