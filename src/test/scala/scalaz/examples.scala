...
package adt {
  ...
  final case object Baz extends Foo {
    ...
    implicit val equal: Equal[Baz.type] =
      TypeclassDerivation[Equal].xderiving0(Baz)
    implicit val default: Default[Baz.type] =
      TypeclassDerivation[Default].xderiving0(Baz)
  }
  object Bar {
    implicit val equal: Equal[Bar] =
      TypeclassDerivation[Equal].xderiving1((s: String) => Bar(s), _.s)
    implicit val default: Default[Bar] =
      TypeclassDerivation[Default].xderiving1((s: String) => Bar(s), _.s)
  }
  object Faz {
    implicit val equal: Equal[Faz] =
      TypeclassDerivation[Equal]
        .xderiving2((b: Boolean, i: Int) => Faz(b, i), f => (f.b, f.i))
    implicit val default: Default[Faz] =
      TypeclassDerivation[Default]
        .xderiving2((b: Boolean, i: Int) => Faz(b, i), f => (f.b, f.i))
  }
  object Foo {
    ...
    implicit val equal: Equal[Foo] =
      TypeclassDerivation[Equal].xcoderiving3(to, from)
    implicit val default: Default[Foo] =
      TypeclassDerivation[Default].xcoderiving3(to, from)
  }
}
...
package recadt {
  ...
  object Leaf {
    implicit val equal: Equal[Leaf] =
      TypeclassDerivation[Equal].xderiving1((v: String) => Leaf(v), _.value)
    implicit val default: Default[Leaf] =
      TypeclassDerivation[Default].xderiving1((v: String) => Leaf(v), _.value)
  }
  object Branch {
  ...
  object ATree {
    ...
    implicit val equal: Equal[ATree] =
      TypeclassDerivation[Equal].xcoderiving2(to, from)
    implicit val default: Default[ATree] =
      TypeclassDerivation[Default].xcoderiving2(to, from)
  }
...
package recgadt {
  ...
  object GLeaf {
    implicit def equal[A: Equal]: Equal[GLeaf[A]] =
      TypeclassDerivation[Equal].xderiving1((v: A) => GLeaf(v), _.value)
    implicit def default[A: Default]: Default[GLeaf[A]] =
      TypeclassDerivation[Default].xderiving1((v: A) => GLeaf(v), _.value)
  }
  object GBranch {
    implicit def equal[A: Equal]: Equal[GBranch[A]] =
      TypeclassDerivation[Equal].xderiving2(
        (left: GTree[A], right: GTree[A]) => GBranch(left, right),
        b => (b.left, b.right)
      )
    implicit def default[A: Default]: Default[GBranch[A]] =
      TypeclassDerivation[Default].xderiving2(
        (left: GTree[A], right: GTree[A]) => GBranch(left, right),
        b => (b.left, b.right)
      )
  }
